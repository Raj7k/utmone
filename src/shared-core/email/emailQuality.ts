...
export const isLikelyWorkEmail = (email: string): boolean => {
  const result = checkEmailQuality(email, { allowDisposable: true });
  return result.ok && result.reason !== "disposable";
};

export const getDisposableDomainSeed = (): string[] => [...DISPOSABLE_SEED];
