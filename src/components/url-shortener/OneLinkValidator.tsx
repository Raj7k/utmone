    ...
    <div className="container max-w-7xl mx-auto py-8 px-4">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        ...
        <TabsContent value="versions">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Track and manage all versions of your links</CardDescription>
            </CardHeader>
            <CardContent>
              {urlGroups && urlGroups.length > 0 ? (
                <div className="space-y-6">
                  {urlGroups.map((group) => (
                    <div key={group.destination_url} className="border rounded-lg p-4">
                      <div className="flex items-start justify-between mb-4">
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs text-secondary-label mt-1">
                            {group.versionCount} versions • {group.totalClicks} total clicks
                          </p>
                        </div>
                      </div>
        ...
        <TabsContent value="analytics">
          {urlGroups && urlGroups.length > 0 ? (
            <div className="space-y-6">
              <AggregateView groups={urlGroups} workspaceId={workspaceId || ''} />
              {urlGroups.map((group) => (
                <VersionAnalytics key={group.destination_url} versions={group.versions} />
              ))}
            </div>
          ) : (
            <Card>
              <CardContent className="text-center py-12 text-secondary-label">
