...
// Cinematic Journey Sankey: Fiber-optic bundles → heartbeat pulse → bloom revenue
export const JourneySankeyVisual = () => {
  ...
  const sources = [
    { y: 12, width: 45, icon: Search, color: "#3B82F6", label: "Paid" },
    { y: 24, width: 38, icon: Users, color: "#10B981", label: "Organic" },
    { y: 36, width: 50, icon: Mail, color: "#8B5CF6", label: "Email" },
    { y: 48, width: 32, icon: MousePointer, color: "#F59E0B", label: "Referral" },
  ];
  ...
  return (
    <svg viewBox="0 0 120 60" className="w-full h-full transform-gpu">
      <defs>
        ...
        {/* Fiber glow */}
        <filter id="journeyFiberGlow" x="-50%" y="-50%" width="200%" height="200%">
        ...
        {sources.map((source, i) => (
          <linearGradient key={`journeyFlow-${i}`} id={`journeyFlow-${i}`} x1="0%" y1="0%" x2="100%" y2="0%">
        ...
        <radialGradient id="journeyGoldRevenue" cx="50%" cy="50%" r="50%">
        ...
        <g key={i}>
          ...
          <foreignObject x="4" y={source.y - 1} width="10" height="8">
            <div className="flex items-center justify-center w-full h-full">
              <source.icon className="w-2 h-2" style={{ color: source.color }} />
            </div>
          </foreignObject>
        </g>
      ))}
        ...
        fiberOffsets.map((offset, strandIdx) => {
          ...
          return (
            <motion.path
              ...
              fill="none"
              stroke={isCenter ? `url(#journeyFlow-${srcIdx})` : source.color}
              strokeWidth={isCenter ? 1 : 0.3}
              strokeLinecap="round"
              strokeOpacity={isCenter ? 0.8 : 0.2}
              filter={isCenter ? "url(#journeyFiberGlow)" : undefined}
