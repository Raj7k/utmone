...
export const LinkPagesVisual = () => {
  ...
  const paths = STAGES.slice(0, -1).map((stage, index) => {
    ...
    return `M ${startX} ${centerY} C ${cp1x} ${centerY - 12}, ${cp2x} ${centerY + 12}, ${endX} ${centerY}`;
  });
  ...
  const getPointOnPath = (pathIndex: number, progress: number) => {
    ...
    const pathEl = svgRef.current.querySelector(`#linkpages-path-${pathIndex}`) as SVGPathElement;
  ...
  return (
    <div className="relative w-full">
      ...
      >
        <defs>
          <filter id="linkpages-particle-glow" x="-100%" y="-100%" width="300%" height="300%">
          ...
          <filter id="linkpages-node-glow" x="-200%" y="-200%" width="500%" height="500%">
          ...
          return (
            <g key={stage.id}>
              <circle
                ...
                strokeWidth="1.5"
                filter={isGlowing ? "url(#linkpages-node-glow)" : undefined}
                style={{ 
                  transform: `scale(${isGlowing ? 1.15 : 1})`,
                  transformOrigin: `${stage.x}px ${centerY}px`
                }}
              />
              <circle cx={stage.x} cy={centerY} r="8" fill="none" className="stroke-white/[0.12]" strokeWidth="1" />
          ...
          return (
            <g key={particle.id}>
              ...
              <circle
                ...
                r="2"
                fill={particleColor}
                filter="url(#linkpages-particle-glow)"
              />
            </g>
          );
        })}
