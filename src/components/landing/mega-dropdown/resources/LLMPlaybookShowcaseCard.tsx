...
export function LLMPlaybookShowcaseCard() {
  return (
    <Link to="/resources/playbooks/llm-ranking" className="block group">
      ...
      >
        ...
        <div className="flex items-center gap-2 mt-4 mb-2">
          ...
          <span className="text-[10px] font-medium text-white/80">LLM Ranking Playbook</span>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          >
            {[1, 2, 3].map((i) => (
              <motion.div
                key={i}
                className="w-1 h-3 bg-white/30 rounded-sm"
                animate={{ scaleY: [0.5, 1, 0.5] }}
                transition={{ duration: 1.5, repeat: Infinity, delay: i * 0.2 }}
              />
            ))}
          </motion.div>
          <span className="text-[9px] text-white/40">rank higher in AI search</span>
        </div>
      </motion.div>
    </Link>
  );
