...
import { Copy, Check, Mail, Tag, Code, Smartphone, ExternalLink, CheckCircle2 } from 'lucide-react';
...
const handleLogin = async (credentials) => {
  ...
  if (user) {
    ${setupOption === 'gtm' ? `dataLayer.push({
      'event': 'user_logged_in',
      ...
      {setupOption === 'developer' && (
        <div className="space-y-4 p-4 bg-blue-500/5 rounded-lg border border-blue-500/20">
          ...
          <p className="text-xs text-muted-foreground">
            click the button below to open an email with ready-to-use instructions. 
            your developer just needs to add one line of code after the login success handler.
          </p>
      ...
      {setupOption === 'self' && (
        <div className="space-y-4 p-4 bg-green-500/5 rounded-lg border border-green-500/20">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-medium text-foreground">add this after login/signup</span>
          </div>
          ...
          <div className="space-y-2 text-xs">
            <p className="font-medium text-foreground">where to add it:</p>
            <ul className="space-y-1 text-muted-foreground">
              <li className="flex items-center gap-2">
                <Smartphone className="h-3 w-3" />
                <span><strong>React/Next.js:</strong> in your <code className="bg-muted px-1 rounded">signIn()</code> success callback</span>
              </li>
              <li className="flex items-center gap-2">
                <Smartphone className="h-3 w-3" />
                <span><strong>Supabase:</strong> after <code className="bg-muted px-1 rounded">supabase.auth.signInWithPassword()</code></span>
              </li>
              <li className="flex items-center gap-2">
                <Smartphone className="h-3 w-3" />
                <span><strong>Firebase:</strong> after <code className="bg-muted px-1 rounded">signInWithEmailAndPassword()</code></span>
              </li>
              <li className="flex items-center gap-2">
                <Smartphone className="h-3 w-3" />
                <span><strong>Auth0:</strong> in the <code className="bg-muted px-1 rounded">onRedirectCallback</code></span>
              </li>
            </ul>
          </div>
          ...
          <div className="p-3 bg-muted/50 rounded-lg">
            <p className="text-xs text-muted-foreground">
              <strong className="text-foreground">example (React):</strong>
            </p>
